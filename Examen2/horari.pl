...
writeClauses(K):-
    eachCourseExactlyOneExam,
    eachWeekAtMost5Exams,
    eachTeacherAtMostOneExamPerWeek,
    eachStudentAtMostTwoExamsPerWeek,
    defineLast,
    minimumLastKstudents(K),
    true.
    
eachCourseExactlyOneExam:-
    course(C),
    findall(courseWeek-C-W, week(W), Lits), exactly(1,Lits), fail.
eachCourseExactlyOneExam.

eachWeekAtMost5Exams:-
    week(W),
    findall(courseWeek-C-W, course(C), Lits), atMost(5,Lits), fail.
eachWeekAtMost5Exams.

eachTeacherAtMostOneExamPerWeek:-
    teacher(T),
    week(W),
    findall(courseWeek-C-W, course(C,T) , Lits), atMost(1,Lits), fail.
eachTeacherAtMostOneExamPerWeek.

eachStudentAtMostTwoExamsPerWeek:-
    student(_,L),
    week(W),
    findall(courseWeek-C-W, member(C,L), Lits), atMost(2,Lits), fail.
eachStudentAtMostTwoExamsPerWeek.
    
defineLast:-
    student(S,L),
    member(C,L),
    writeClause([\+courseWeek-C-3, last-S]),
    fail.
defineLast.

minimumLastKstudents(K):-
    findall(last-S, student(S), Lits), atMost(K,Lits), fail.
minimumLastKstudents(_).
