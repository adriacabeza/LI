...
noWorkInRange:- nurseIDandBlocking(N,HS,HE), hourInRange(HS,HE,H), writeClause([\+worksNH-N-H]), fail.
noWorkInRange.

nurseType:- nurse(N), findall(nurseType-N-T, type(T), Lits), exactly(1, Lits), fail.
nurseType.

startsWorkOnly1:- nurse(N), findall( startsNH-N-H, hour(H) , Lits), exactly(1,Lits), fail.
startsWorkOnly1.

atLeastNeededNurses:- hour(H), needed(H,M),findall( worksNH-N-H, nurse(N) , Lits), atLeast(M,Lits), fail.
atLeastNeededNurses.
...
nurseWork:- 
    ...
    subtract(Lits2,Lits1,Lits),negateAll(Lits,Nlits),
    expressIfThen(startsNH-N-H, nurseType-N-T, Lits1),
    expressIfThen(startsNH-N-H, nurseType-N-T, Nlits),  
    fail.
nurseWork.

expressIfThen(S, T, Lits):-
    member(Lit,Lits),
    writeClause([\+S,\+T,Lit]),
    fail. 
expressIfThen(_,_,_).
