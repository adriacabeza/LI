...
writeClauses:- setPiece, fillsBoard, noOverlapping.

setPiece :- rect(B), findall(starts-B-X-Y, fitsBoard(B, X, Y), L), exactly(1, L), fail.
setPiece. % Force true eval.

fitsBoard(B, X1, Y1) :- rect(B, W, H), insideTable(X1, Y1), X2 is X1+W-1, Y2 is Y1+H-1, insideTable(X2, Y2).

fillsBoard :-
	rect(B, W, H), fitsBoard(B, X1, Y1),
	X2 is X1+W-1, Y2 is Y1+H-1,
	between(X1, X2, XC), between(Y1, Y2, YC),
	writeClause([\+starts-B-X1-Y1, fills-B-XC-YC]), fail.
fillsBoard.

noOverlapping :- rect(B1), rect(B2), B1\=B2, insideTable(X, Y), atMost(1, [fills-B1-X-Y, fills-B2-X-Y]), fail.
noOverlapping.
...
displaySol(M):-
	yCoord(Y1), height(H),
	Y2 is H-Y1+1, nl, xCoord(X),
	member(fills-B-X-Y2, M), writeNumber(B), fail.
displaySol(_).

writeNumber(B):- B < 10, write(" "). 
writeNumber(B):- write(B), write(" "). 
