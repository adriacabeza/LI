...
tsp( Cities, AccumulatedKm, [ CurrentCity | _ ] ):- bestRouteSoFar(Km,_), lowerBound(Cities, CurrentCity, LB), AccumulatedKm + LB >= Km, !, fail.
...
lowerBound([City], CurrentCity, LB) :- distance(City, CurrentCity, LB), !.
lowerBound([City|Cities], CurrentCity, LB) :- lowerBound(Cities, CurrentCity, LB0), distance(City, CurrentCity, D), (LB0 < D -> LB is LB0 ; LB is D).
