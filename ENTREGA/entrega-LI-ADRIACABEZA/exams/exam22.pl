...
writeClauses:-   
    eachNeighBorAtLeastN,
    eachReprDifferentNeigh,
    exactlyNumReps,
...
eachNeighBorAtLeastN:- neighbor(N),neighborRepresentedBy(N,L), findall(chosen(R),member(R,L),Lits), atLeast(1,Lits),fail.
eachNeighBorAtLeastN.

eachReprDifferentNeigh:- neighbor(N1), neighbor(N2), N1 < N2, neighborNeighborhood(N1,NH1), neighborNeighborhood(N2,NH2), NH2 = NH1, writeClause([-chosen(N1),-chosen(N2)]),fail.
eachReprDifferentNeigh.
