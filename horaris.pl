...
%exactly 1 of the possible professors is assigned
exactly1ValidProfessorPerCourse:- course(C), courseProfessors(C, LP), findall(cp(C,P), member(P, LP), Lits), exactly(1, Lits), fail.
%forcing the unavailable professors to not be assigned.
...
%
crAndcdh2cdhr:- course(C), courseRooms(C, Rs), member(R, Rs), day(D), hour(H), writeClause([-cr(C, R), -cdh(C, D, H), cdhr(C, D, H, R)]), fail.
crAndcdh2cdhr.
...
sameYearNoOverlap:- day(D), hour(H), year(Y), findall(cdh(C, D, H), courseYear(C, Y), Lits), atMost(1, Lits), fail.
sameYearNoOverlap.
